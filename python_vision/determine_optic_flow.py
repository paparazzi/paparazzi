...
def determine_optic_flow(filename_1, filename_2, method='Harris', max_points = 100, graphics = True):
    ...
    if(method == 'Harris'):
        ...
        n_points = len(inds[0])
        corners_old = np.float32(np.zeros([n_points, 2]));

        corners = np.stack((inds[1].T, inds[0].T), axis=1)
        print(corners)

        for i in range(n_points):
            corners_old[i, 0] = inds[1][i];
            corners_old[i, 1] = inds[0][i];
    ...
    if(graphics):
        ...
        for p in range(n_corners):
            cv2.arrowedLine(im, (int(corners[p, 0]), int(corners[p, 1])), (int(corners_new[p, 0]), int(corners_new[p, 1])), color, thickness=2, tipLength=0.5)
