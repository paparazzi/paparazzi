...
public class ajaxRqst extends HttpServlet {
			 ...
			 if (order.equals("fpl_update"))
			 {
				 ...
				 try{
					 ...
					 if (!doIt){ 	
						 	...
						 	serializer.setOutputCharStream(new java.io.FileWriter(default_folder + "/conf/"+"immat.xml"));
	...
	{   Conf myConf = new Conf(default_folder,"pow.conf");
		DatagramSocket socket;
		try {
			System.out.println("attempt to send order to ivy");
			socket = new DatagramSocket(myConf.portWebToIvy());
			socket.setSoTimeout(myConf.timeout());
			ServletContext srvCtxt = getServletContext();
			if (srvCtxt==null) System.out.println("srvCtxt null !!?");
