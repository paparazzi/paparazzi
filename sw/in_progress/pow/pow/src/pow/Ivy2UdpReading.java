 ...
 * @version 2.0
 */
class Ivy2UdpReading implements Runnable{
	 ...
	 */
	public Ivy2UdpReading(String hostname,String login, String password)
	 {
		num_msg = 0;
		...
		try{
		 ...
		 // ********************************************* //
		  this.url =  "http://"+hostname+":8080/TestServletPow/Ivy2TomcatHttpServer.srv";
		 // ********************************************* //
	  ...
	  * @param password
	  */
	 public Ivy2UdpReading(PowUrl urlObj,String login, String password)
	 {
		 stopThread = false;
		try{
			this.login = login;
	  ...
	  * @SEE blogs.sun.com/gc/entry/unable_to_find_valid_certification
	 ...
	 public void getWebId() throws IvyConnectionExeption
	 {
		 ...
		 //si https ...
		 // inutile mais bon pour 1 connection ...
    ...
    public static void main(String argv[]) throws IvyException {
    	...
    	if (argv.length<1){
    	 System.out.println("failure :1 params required : [server name or url]");	
        	...
        	int true_length_of_msg ;
        	int i;
        	byte[] data_with_no_padding={'z'};
        	try {
        		 // seek user login info 
        		String hostname = argv[0];
        		PowUrl connectInfo = new PowUrl("http","blanc",8080,"TestServletPow","Ivy2TomcatHttpServer.srv");
        		System.out.println("connecting to : "+ connectInfo.getWebUrl());
        		
        		String login="admin_ivy";
    			String password = "pwdadmin_ivy";
            	Ivy2UdpReading link = new Ivy2UdpReading(connectInfo,login, password);
				link.getWebId(); // login procedure
				//
				link.setStorage();
					...
					while (true) {
				            ...
				            try{
					            ...
					            try{
									...
									ex.printStackTrace();
									System.out.println("order := "+(new String(Hex.encodeHex(data_with_no_padding))));
									System.out.print("cannot decrypt msg : msg not sended to server");
								}
								catch(IllegalBlockSizeException ex){
									ex.printStackTrace();
									System.out.println("order := "+(new String(Hex.encodeHex(data_with_no_padding))));
