...
void on_reset_changed(GtkButton *button, gpointer user_data)
{
...
GtkWidget *build_gui(void)
{
  ...
  gtk_table_attach(GTK_TABLE(table1), label3, 0, 1, 3, 4, (GtkAttachOptions)(GTK_FILL), (GtkAttachOptions)(0), 0, 0);
  ...
  gtk_table_attach(GTK_TABLE(table1), radiobutton1, 1, 2, 0, 1, (GtkAttachOptions)(GTK_FILL), (GtkAttachOptions)(0), 0,
                   0);
  ...
  gtk_table_attach(GTK_TABLE(table1), radiobutton2, 2, 3, 0, 1, (GtkAttachOptions)(GTK_FILL), (GtkAttachOptions)(0), 0,
                   0);
  gtk_radio_button_set_group(GTK_RADIO_BUTTON(radiobutton2), radiobutton1_group);
  radiobutton1_group = gtk_radio_button_get_group(GTK_RADIO_BUTTON(radiobutton2));

  button3 = gtk_button_new_with_mnemonic("Set Home Here");
  gtk_widget_show(button3);
  gtk_table_attach(GTK_TABLE(table1), button3, 3, 4, 0, 1, (GtkAttachOptions)(GTK_FILL), (GtkAttachOptions)(0), 0, 0);
  //gtk_widget_set_focus_on_click(button3, TRUE);
  //gtk_button_set_group(GTK_BUTTON (button3), button1_group);
  //button1_group = gtk_radio_button_get_group(GTK_RADIO_BUTTON (button3));
  ...
  gtk_table_attach(GTK_TABLE(table1), azim_scale, 1, 3, 1, 2, (GtkAttachOptions)(GTK_EXPAND | GTK_FILL),
                   (GtkAttachOptions)(GTK_FILL), 0, 0);
  //gtk_range_set_update_policy(GTK_RANGE (azim_scale), GTK_UPDATE_DELAYED);

  elev_scale = gtk_hscale_new(GTK_ADJUSTMENT(gtk_adjustment_new(0, 0, 90, 1, 1, 1)));
  //elev_scale = gtk_hscale_new(GTK_ADJUSTMENT (gtk_adjustment_new (45, 0, 90, 1, 1, 1)));
  gtk_widget_show(elev_scale);
  gtk_table_attach(GTK_TABLE(table1), elev_scale, 1, 3, 2, 3, (GtkAttachOptions)(GTK_FILL), (GtkAttachOptions)(GTK_FILL),
                   0, 0);
  //gtk_range_set_update_policy(GTK_RANGE (elev_scale), GTK_UPDATE_DELAYED);

  entry1 = gtk_entry_new();
  gtk_widget_show(entry1);
  gtk_table_attach(GTK_TABLE(table1), entry1, 1, 3, 3, 4, (GtkAttachOptions)(GTK_EXPAND | GTK_FILL),
                   (GtkAttachOptions)(0), 0, 0);

  g_signal_connect((gpointer) button3, "pressed", G_CALLBACK(on_reset_changed), NULL);
  g_signal_connect((gpointer) radiobutton1, "toggled", G_CALLBACK(on_mode_changed), NULL);
  g_signal_connect((gpointer) azim_scale, "value-changed", G_CALLBACK(on_azimuth_changed), NULL);
  g_signal_connect((gpointer) elev_scale, "value-changed", G_CALLBACK(on_elevation_changed), NULL);
...
#define GLADE_HOOKUP_OBJECT_NO_REF(component,widget,name) \
  ...
  gtk_entry_set_text(GTK_ENTRY(entry1), "Waiting for GPS lock!");
...
void set_servos(void)
{
  ...
  g_message("vservo %i hservo %i", (int)(vservo / 4),
            (int)(hservo / 4)); //Divide by 4 so we can have the servo PW with 1 microsecond resolution.
...
void on_GPS_STATUS(IvyClientPtr app, void *user_data, int argc, char *argv[])
{
  if (home_found == 0) {
    if (atof(argv[0]) == 3) { /* wait until we have a valid GPS fix */
      ...
      home_found = 1;
      gtk_entry_set_text(GTK_ENTRY(entry1), gps_lock);
    } else {
      gtk_entry_set_text(GTK_ENTRY(entry1), "Waiting for GPS lock!");
    }
  }
...
int main(int argc, char **argv)
{
  ...
  IvyBindMsg(
    on_GPS_STATUS, NULL, "^\\S* GPS (\\S*) (\\S*) (\\S*) (\\S*) (\\S*) (\\S*) (\\S*) (\\S*) (\\S*) (\\S*) (\\S*)");
