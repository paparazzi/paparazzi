...
(* ============================================================================= *)
(* = Fonction interne de creation d'une boite de capture d'ecran               = *)
(* ============================================================================= *)
let creation_fen_capture default_filename default_format tooltips with_caption =
  let filename = ref default_filename and format = ref default_format in
  filename := Gtk_image.set_filename_extension !filename default_format ;

  let lst_formats = [Gtk_image.PNG; Gtk_image.JPEG; Gtk_image.POSTSCRIPT;
					 Gtk_image.TIFF; Gtk_image.BMP; Gtk_image.PPM] in
  (* Correction du format par defaut suivant qu'il est disponible ou pas *)
  (* sur l'architecture courante *)
  let default_format =
    if Gtk_image.is_format_capture_dispo default_format then default_format else begin
      let rec f lst =
		match lst with
		  typ::reste ->
			if Gtk_image.is_format_capture_dispo typ then typ else f reste
		| [] -> default_format
      in
      let typ = f lst_formats in
      filename := Gtk_image.update_extension_capture !filename !format typ ;
      typ
    end
  in
  format := default_format ;

  let get_caption = ref (fun () -> None) in

  (* Creation de la boite de dialogue *)
  let height = if with_caption then 250 else 210 in
  let window = GWindow.dialog ~title:"Capture d'ecran"
      ~border_width:10 ~width:350 ~height:height () in
  let vbox = window#vbox in

  (* Legende *)
  let taille_x = 20 and taille_y = 10 in

  if with_caption then begin
	let color_caption = ref (`RGB(65535, 65535, 65535)) and
	...
	(* Fonction de creation d'un bouton de selection d'une couleur *)
	let create_but col help_text pack_method redraw_func =
	  let but = create_color_selection_button window taille_x taille_y !col
		  pack_method (fun color -> col := color;	redraw_func ()) in
	  match tooltips with
		None -> ()
	  | Some t -> add_tooltips t but help_text
	in

	(* Creation de tous les boutons de selection des couleurs de la legende *)
	let rec create_boutons () =
	  !hb#destroy () ;
	  hb := create_hbox (hbox#pack ~from:`END) ;
	  create_but contour_color "Couleur du texte de la legende"
		!hb#pack create_boutons;
	  create_but back_color    "Couleur du fond de la boite de legende"
		!hb#pack create_boutons;
	  create_but color_caption "Couleur du contour de la boite de legende"
		!hb#pack create_boutons;
	in

	create_boutons () ;
	set_sensitive !hb false ;

	(* Masquage des boutons de selection de couleur si pas de legende *)
	ignore(entry_caption#connect#changed ~callback:(fun () ->
	  set_sensitive !hb (entry_caption#text <> ""))) ;
	(match tooltips with
	  None -> () | Some t -> add_tooltips t entry_caption
		  "Texte de la legende (rien=pas de legende)") ;

	(* Recuperation de la legende *)
	get_caption := fun () ->
	  let caption = entry_caption#text in
	  if caption = "" then None
	  else Some (caption, !color_caption, !contour_color, !back_color,
				 get_fixed_font2 ())
  end ;
  ...
  let lst_but = List.fold_left (fun lst typ ->
	...
	let but =
	  if lst = [] then GButton.radio_button ~label:name ~packing:hbox#add ()
	  else GButton.radio_button ~label:name ~packing:hbox#add ~group:(List.hd lst)#group ()
	in
	but#set_active (typ = default_format) ;
	set_sensitive but (Gtk_image.is_format_capture_dispo typ) ;
	begin
	  match tooltips with
		None -> ()
	  |	Some t ->
		  add_tooltips t but
			("Capture au format "^(Gtk_image.extended_string_of_format_capture typ))
	end ;
	lst @ [but]) [] lst_formats in
  ...
  let but_ok      = create_button "Capture" window#action_area#add and
	  but_cancel  = create_button "Fermer"  window#action_area#add in
  but_connect but_cancel (fun () -> window#destroy ()) ;
  but_ok#grab_default ();

  (* Fonction appelee lors de la sauvegarde pour afficher la progression *)
  let current_progress = ref "" in
  let progression_save etape compteur =
	begin
	  match etape with
		Gtk_image.INIT     -> lab_save#set_text "Sauvegarde en cours..." ;
	  | Gtk_image.SAVING   -> lab_save#set_text "Sauvegarde en cours..." ;
	  | Gtk_image.FINISHED -> lab_save#set_text "OK" ;
	end ;
	let p = Printf.sprintf "%.0f%%" (100.*.compteur) in
	(* Comme ca on n'affiche que tous les 1%. Sinon c'est tres lent avec GTK2 *)
	if !current_progress<>p then begin
	  pbar_save#set_text p ;
	  pbar_save#set_fraction compteur ; force_update_interface () ;
	  current_progress:=p
	end
  in
  ...
  begin
	match tooltips with
	  None -> ()
	| Some t ->
		add_tooltips t but_ok     "Effectue la capture d'ecran" ;
		add_tooltips t but_cancel "Ferme la fenetre" ;
		add_tooltips t entry      "Nom du fichier de capture" ;
		add_tooltips t but_fic    "Selection d'un nom de fichier" ;
  end ;

  (* Connexion des callbacks *)
  List.iter2 (fun b typ ->
	ignore(b#connect#clicked ~callback:(fun () ->
	  filename := Gtk_image.update_extension_capture !filename !format typ ;
	  format := typ ;
	  entry#set_text !filename))) lst_but lst_formats ;

  but_connect but_fic (fun () ->
	open_file_dlg "Fichier image"  (fun f ->
	  filename := Gtk_image.set_filename_extension f !format ;
	  entry#set_text !filename)
	  None !filename true) ;

  ignore(entry#connect#changed ~callback:(fun () -> filename:=entry#text)) ;

  (* On renvoie les divers parametres et widgets *)
  (filename, format, Some progression_save, window, but_ok, entry, !get_caption)

(* ============================================================================= *)
(* = Boite de capture d'ecran sans legende                                     = *)
(* =                                                                           = *)
(* = default_filename = nom du fichier par defaut                              = *)
(* = default_format   = format selectionne par defaut (cf capture.ml)          = *)
(* = drawable         = zone (fenetre ou pixmap) contenant le dessin           = *)
(* = tooltips         = None ou Some t                                         = *)
(* = x                = coordonnee x du point de depart dans la zone           = *)
(* = y                = coordonnee y de ce point                               = *)
(* = width            = largeur de la zone capturee                            = *)
(* = height           = hauteur de la zone capturee                            = *)
(* ============================================================================= *)
let screenshot_box default_filename default_format drawable
	tooltips x y width height =

  let (filename, format, progression_save, window, but_ok, entry, _) =
	creation_fen_capture default_filename default_format tooltips false in

  let do_capture () =
	if (String.length !filename)>0 then
	  Gtk_image.capture_part drawable x y width height !filename !format progression_save
  in

  but_connect but_ok do_capture ;

  ignore(entry#connect#activate ~callback:(fun () ->
	filename:=Gtk_image.set_filename_extension !filename !format ;
	entry#set_text !filename ;
	do_capture ())) ;
...
(* ============================================================================= *)
(* = Boite de capture d'ecran sans legende qui lance une fonction apres la     = *)
(* = capture                                                                   = *)
(* =                                                                           = *)
(* = default_filename = nom du fichier par defaut                              = *)
(* = default_format   = format selectionne par defaut (cf capture.ml)          = *)
...
(* = width            = largeur de la zone capturee                            = *)
(* = height           = hauteur de la zone capturee                            = *)
(* = after_func       = fonction a lancer apres la capture                     = *)
(* ============================================================================= *)
let screenshot_box_with_func default_filename default_format drawable
	tooltips x y width height after_func =
  ...
  let do_capture () =
	if (String.length !filename)>0 then
	  Gtk_image.capture_part drawable x y width height !filename !format
		progression_save ;
	after_func ()
 in
...
(* ============================================================================= *)
(* = Boite de capture d'ecran avec legende                                     = *)
(* = Attention, ne fonctionne qu'avec des pixmaps a cause de la legende        = *)
(* =                                                                           = *)
(* = default_filename = nom du fichier par defaut                              = *)
(* = default_format   = format selectionne par defaut (cf capture.ml)          = *)
(* = window           = la fenetre mere de la zone suivante                    = *)
(* = drawable         = zone (pixmap uniquement) contenant le dessin           = *)
(* = tooltips         = None ou Some t                                         = *)
(* = x                = coordonnee x du point de depart dans la zone           = *)
(* = y                = coordonnee y de ce point                               = *)
(* = width            = largeur de la zone capturee                            = *)
(* = height           = hauteur de la zone capturee                            = *)
(* ============================================================================= *)
let screenshot_box_with_caption default_filename default_format
	drawable tooltips x y width height =

  let (filename, format, progression_save, window, but_ok, entry, get_caption) =
	creation_fen_capture default_filename default_format tooltips true in

  let do_capture () = 
	if (String.length !filename)>0 then
	  Gtk_image.capture_part_with_caption window drawable x y width height !filename !format
		progression_save (get_caption())
  in
