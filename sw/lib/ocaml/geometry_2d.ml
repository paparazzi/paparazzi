...
(* Projection of a point on a line, segment, set segments *)
...
let in_tesselation poly =
  ...
  while !n>=3 do
    ...
    while not !finished && not !earfound do
      if vertices.(!v2).ear then begin
        ...
        (* It is not CCW and therefore bp external display is normal ??? *)
        ...
        (* Removed the point v2 *)
        vertices.(v1).next <- v3 ;
        vertices.(v3).prev <- v1 ;
        current_idx:=v3 ;

        (* A triangle below to search *)
        decr n
  ...
  (* If the polygon was departing CW, it was back and the numbers          *)
  (* Points are then not in the same sense that the polygon passes         *)
  (* By the user. It then puts the numbers as they etaients at             *)
  (* Call a triangulation function                                         *)
...
(* ============================================================================= *)
(* = Search fan triangles in a list of triangless                   = *)
(* ============================================================================= *)
...
(* Performs triangulation of the polygon
   triangle_fan OpenGL. Output is returned a list containing lists
   points. Each point lists contains 3 points (triangle)
