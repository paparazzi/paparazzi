...
let transform_values = fun attribs_not_modified env attribs ->
  List.map
    (fun (a, v) ->
      let v' =
        if List.mem (Compat.string_lowercase a) attribs_not_modified
        then v
...
let prefix_or_deroute = fun prefix reroutes name attribs ->
  List.map
    (fun (a, v) ->
      let v' =
        if Compat.string_lowercase a = name then
  ...
  let rec tr = fun xml ->
    match xml with
        Xml.Element (tag, attribs, children) -> begin
          match Compat.string_lowercase tag with
              "exception" ->
...
let remove_attribs = fun xml names ->
  List.filter (fun (x,_) -> not (List.mem (Compat.string_lowercase x) names)) (Xml.attribs xml)
...
let process_stage = fun stage waypoints ->
  let rec do_it = fun stage ->
    match Compat.string_lowercase (Xml.tag stage) with
        "go" | "stay" | "circle" ->
