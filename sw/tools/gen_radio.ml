...
let norm1_ppm = fun c ->
  if c.neutral = c.min then
    sprintf "tmp_radio * (MAX_PPRZ / (%d-%d))" c.max c.min, "0"
  else
    sprintf "tmp_radio * (tmp_radio >=0 ? (MAX_PPRZ/(%d-%d)) : (MIN_PPRZ/(%d-%d)))" c.max c.neutral c.min c.neutral, "MIN_PPRZ"
  ...
  List.iter
    (fun c ->
      ...
      if c.averaged then begin
        ...
        printf "  tmp_radio = USEC_OF_RC_PPM_TICKS(_ppm[RADIO_%s]) - %d;\\\n" c.name c.neutral;
...
let norm1_ppm2 = fun c ->
  if c.neutral = c.min then
    ...
    sprintf "(tmp_radio >=0 ? (tmp_radio *  MAX_PPRZ) / (%d-%d) : (tmp_radio * MIN_PPRZ) / (%d-%d))" c.max c.neutral c.min c.neutral, "MIN_PPRZ"
