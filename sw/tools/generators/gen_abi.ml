...
module Gen_onboard = struct
  ...
  let print_message_id = fun h messages ->
    ...
    List.iter (fun msg ->
      ...
      Printf.fprintf h "#define ABI_%s_ID %d\n" (Compat.string_capitalize msg.name) msg.id
    ) messages;
    !highest_id
  ...
  let print_callbacks = fun h messages ->
    Printf.fprintf h "\n/* Callbacks */\n";
    List.iter (fun msg ->
      Printf.fprintf h "typedef void (*abi_callback%s)" (Compat.string_capitalize msg.name);
  ...
  let print_msg_bind = fun h msg ->
    let name = Compat.string_capitalize msg.name in
