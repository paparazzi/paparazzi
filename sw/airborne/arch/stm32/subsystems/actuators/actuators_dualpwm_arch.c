 ...
 */
static inline void actuators_dualpwm_arch_channel_init(uint32_t timer_peripheral,
...
static inline void set_servo_timer(uint32_t timer, uint32_t period, uint8_t channels_mask) {
  ...
  if (bit_is_set(channels_mask, 0)) {
    actuators_dualpwm_arch_channel_init(timer, TIM_OC1);
  }
  if (bit_is_set(channels_mask, 1)) {
    actuators_dualpwm_arch_channel_init(timer, TIM_OC2);
  }
  if (bit_is_set(channels_mask, 2)) {
    actuators_dualpwm_arch_channel_init(timer, TIM_OC3);
  }
  if (bit_is_set(channels_mask, 3)) {
    actuators_dualpwm_arch_channel_init(timer, TIM_OC4);
  }
