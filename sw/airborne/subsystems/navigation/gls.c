...
// calculate ground speed on final (target_speed - head wind)
  ...
  float wind_on_final = wind_norm * (((td_af_x*wind->y)/(td_af*wind_norm))+((td_af_y*wind->x)/(td_af*wind_norm)));
...
// calculate requiered decent rate on glideslope
  float pre_climb_glideslope = *stateGetHorizontalSpeedNorm_f() * (-tanf(app_angle));
...
// calculate intercept
  float nav_intercept_progress = ((stateGetPositionEnu_f()->x - WaypointX(_sd)) * 2*sd_tod_x + (stateGetPositionEnu_f()->y - WaypointY(_sd)) * 2*sd_tod_y) / Max((sd_intercept*sd_intercept),1.);
  Bound(nav_intercept_progress,-1,1);
  float alt_intercept = WaypointAlt(_tod) - (0.5 * app_intercept_rate * (nav_intercept_progress*sd_intercept/gs_on_final)*(nav_intercept_progress*sd_intercept/gs_on_final));
  float pre_climb_intercept = -nav_intercept_progress * *stateGetHorizontalSpeedNorm_f() * (tanf(app_angle));
...
// distance
  float f_af = sqrt((stateGetPositionEnu_f()->x - WaypointX(_af))*(stateGetPositionEnu_f()->x - WaypointX(_af))+(stateGetPositionEnu_f()->y - WaypointY(_af))*(stateGetPositionEnu_f()->y - WaypointY(_af)));

if(f_af<app_distance_af_sd){ // approach fix (AF) to start descent (SD)
  alt = WaypointAlt(_af);
  pre_climb = 0.;
}
else if((f_af>app_distance_af_sd) && (f_af<(app_distance_af_sd+sd_intercept))){ // start descent (SD) to intercept
  alt = alt_intercept;
