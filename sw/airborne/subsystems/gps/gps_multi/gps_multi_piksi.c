...
#include "subsystems/gps/gps_multi/gps_multi_piksi.h"
                                 ...
                                 void *context __attribute__((unused)))
{
  ...
  if(pos_llh.flags > 0 || last_flags == 0) {
    ...
    LLA_FLOAT_OF_BFP(lla_f, gps_piksi.lla_pos);
    struct UtmCoor_f utm_f;
    utm_f.zone = nav_utm_zone0;
    /* convert to utm */
    utm_of_lla_f(&utm_f, &lla_f);
    /* copy results of utm conversion */
    gps_piksi.utm_pos.east = utm_f.east * 100;
    gps_piksi.utm_pos.north = utm_f.north * 100;
    gps_piksi.utm_pos.alt = gps_piksi.lla_pos.alt;
    gps_piksi.utm_pos.zone = nav_utm_zone0;
...
/* 
 * find out fix. When RTK is established, piksi will send pseudoabsolute position solutions
 * relative to the surveyed base station location as well as send a single point position.
 * Once RTK has been established, however, we'll end up with messages for spp and RTK at the
 * same time. Thus, get_fix_mode() should hold some past values 
 */
...
void piksi_gps_impl_init(void)
{
  ...
  //gps_piksi.nb_channels = GPS_NB_CHANNELS;
...
{ 
  ...
  // if ( get_sys_time_msec() - time_since_last_heartbeat > HEARTBEAT_TIMEOUT ) {
  //   gps_piksi.fix = GPS_FIX_NONE;
  // }
