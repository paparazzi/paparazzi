 ...
 * Should be used with @ahrs_extern_quat AHRS subsystem.
 *
...
void imu_impl_init(void) { 
  ...
  // FIXME: it is necessary to wait for GX3 to power up for proper initialization
...
*/
   ...
   //DEBUG
   //GX3Link(Transmit(0b00000100));  //set options byte 8 - BAD dropping packets
   //GX3Link(Transmit(0b01100011));  //set options byte 7 - BAD
   //DEBUG 
  ...
  */
  ...
  // FIXME: Another wait loop for proper GX3 init
...
void GX3_packet_read_message(void) {
    ...
    ACCELS_BFP_OF_REAL(imu.accel, GX3_accel); // integer
    imuf.accel = GX3_accel;

	  // Rates 
  	//RATES_BFP_OF_REAL(imu.gyro, GX3_rate); // Rates are in rad/s
    ...
    //QUAT_BFP_OF_REAL(ahrs_impl.ltp_to_imu_quat, GX3_quat); 
    ...
    //DEBUG
    //for (uint8_t i = 0;i<GX3_MSG_LEN;i++){
    //	GpsLink(Transmit(GX3_packet.msg_buf[i]));
    //}
}
