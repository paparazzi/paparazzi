...
static inline void b2_gv_adapt_run(int32_t zdd_meas, int32_t thrust_applied) {
  ...
  int32_t meas = ((int32_t)BOOZ_INT_OF_FLOAT(9.81, IACCEL_RES) - zdd_meas) / thrust_applied;
  int32_t residual = meas - b2_gv_adapt_X;
  int32_t E = b2_gv_adapt_P + B2_GV_ADAPT_MEAS_NOISE;
  int32_t K = b2_gv_adapt_P / E;
  b2_gv_adapt_P = b2_gv_adapt_P - K * b2_gv_adapt_P;
  b2_gv_adapt_X = b2_gv_adapt_X + K * residual;
}
