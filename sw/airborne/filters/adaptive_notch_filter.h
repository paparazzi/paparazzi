 ...
 */
static inline void init_adaptive_notch_filter(struct AdaptiveNotchFilter *filter, float sample_time,
 ...
 */
static inline void update_adaptive_notch_filter(struct AdaptiveNotchFilter *filter, float value, float *output_signal)
{
  ...
  float out_est = filter->xband[0] - filter->a_est * filter->xband[1] + filter->xband[2] + filter->r * filter->a_est * filter->y_est[0] - filter->r2 * filter->y_est[1];
