...
void guidance_indi_init_filters(struct FloatEulers *eulers)
{
  ...
  init_butterworth_2_low_pass(&yaw_delta_filt, tau, sample_time, 0.0f);
  previous_yaw_raw = eulers->psi;
  yaw_filt = eulers->psi;
  init_butterworth_2_low_pass(&accely_filt, tau, sample_time, 0.0f);
...
void guidance_indi_propagate_filters(struct FloatEulers *eulers)
{
  ...
  float yaw_delta = eulers->psi - previous_yaw_raw;
  ...
  yaw_filt += get_butterworth_2_low_pass(&yaw_delta_filt);
