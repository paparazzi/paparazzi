...
static void write_reg(struct mt9v117_t *mt, uint16_t addr, uint32_t val, uint16_t len)
{
  ...
  // Transmit the buffer
  i2c_blocking_transmit(mt->i2c_periph, &mt->i2c_trans, MT9V117_ADDRESS, len + 2, 0.1);
}
...
static uint32_t read_reg(struct mt9v117_t *mt, uint16_t addr, uint16_t len)
{
  ...
  i2c_blocking_transceive(mt->i2c_periph, &mt->i2c_trans, MT9V117_ADDRESS, 2, len, 0.1);
...
static inline void mt9v117_write_patch(struct mt9v117_t *mt)
{
  ...
  for (uint8_t i = 0; i < MT9V117_PATCH_LINE_NUM; ++i) {
    ...
    // Transmit the buffer
    i2c_blocking_transmit(mt->i2c_periph, &mt->i2c_trans, mt->i2c_trans.slave_addr, mt9v117_patch_lines[i].len, 0.1);
  }
