...
struct RdybSerialPort* serial_port_new( void ) {
  struct RdybSerialPort* me = malloc(sizeof(struct RdybSerialPort));
  return me;
}

void serial_port_free(struct RdybSerialPort* me) {
  free(me);
}

int  serial_port_open(struct RdybSerialPort* me, const char* device,
...
void serial_port_close(struct RdybSerialPort* me) {
