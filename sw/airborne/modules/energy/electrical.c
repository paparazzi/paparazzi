...
static bool electrical_preflight(char *error_msg) {
  if(electrical.vsupply < TAKEOFF_BAT_LEVEL) {
    snprintf(error_msg, 240, "Battery level %.2fV below minimum takeoff level %.2fV", electrical.vsupply, TAKEOFF_BAT_LEVEL);
    return false;
  }
