...
static void send_gvf(struct transport_tx *trans, struct link_device *dev)
{
    ...
    pprz_msg_send_GVF(trans, dev, AC_ID, &gvf_error, &gvf_traj_type,
            &gvf_s, plen, gvf_param);
}

static void send_circle(struct transport_tx *trans, struct link_device *dev)
{
  if (gvf_traj_type == 1 && (gvf_param[2] == gvf_param[3])) {
    pprz_msg_send_CIRCLE(trans, dev, AC_ID,
                         &gvf_param[0], &gvf_param[1], &gvf_param[2]);
  }
}
...
bool gvf_ellipse(uint8_t wp, float a, float b, float alpha)
{
    ...
    gvf_traj_type = 1;
    gvf_param[0] = waypoints[wp].x;
    gvf_param[1] = waypoints[wp].y;
    gvf_param[2] = a;
    gvf_param[3] = b;
    gvf_param[4] = alpha;
