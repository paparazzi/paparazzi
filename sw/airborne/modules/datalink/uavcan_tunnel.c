...
static int      uavcan_dl_check_free_space(struct uavcan_tunnel_periph* up, long *fd, uint16_t len);
static void     uavcan_dl_put_byte(struct uavcan_tunnel_periph* up, long fd, uint8_t c);
static void     uavcan_dl_put_buffer(struct uavcan_tunnel_periph* up, long fd, const uint8_t *data, uint16_t len);
static void     uavcan_dl_send_message(struct uavcan_tunnel_periph* up, long fd);
static int      uavcan_dl_char_available(struct uavcan_tunnel_periph* up);
static uint8_t  uavcan_dl_get_byte(struct uavcan_tunnel_periph* up);
void uavcan_dl_set_baudrate(struct uavcan_tunnel_periph* up, uint32_t baudrate);

static void uavcan_dl_cb(struct uavcan_iface_t *iface, CanardRxTransfer *transfer);

void uavcan_dl_init_periph(struct uavcan_tunnel_periph* ucdl);

void uavcan_dl_init(void) {
  uavcan_dl_init_periph(&uavcan_tunnel0, UAVCAN_TUNNEL_CHANNEL_ID);
}

void uavcan_dl_init_periph(struct uavcan_tunnel_periph* ucdl, uint8_t channel_id) {
  ucdl->channel_id = channel_id;
  ucdl->iface = &UAVCAN_DL_IFACE;
  ...
  uavcan_bind(UAVCAN_TUNNEL_BROADCAST_ID, UAVCAN_TUNNEL_BROADCAST_SIGNATURE, &ucdl->tunnel_brd_ev, &uavcan_dl_cb);

  // setup device
  ucdl->device.check_free_space = (check_free_space_t)uavcan_dl_check_free_space;
  ucdl->device.put_byte = (put_byte_t)uavcan_dl_put_byte;
  ucdl->device.put_buffer = (put_buffer_t)uavcan_dl_put_buffer;
  ucdl->device.send_message = (send_message_t)uavcan_dl_send_message;
  ucdl->device.char_available = (char_available_t)uavcan_dl_char_available;
  ucdl->device.get_byte = (get_byte_t)uavcan_dl_get_byte;
  ucdl->device.set_baudrate = (set_baudrate_t)uavcan_dl_set_baudrate;
...
static void uavcan_dl_put_byte(struct uavcan_tunnel_periph* up, long fd, uint8_t c) {
  uavcan_dl_put_buffer(up, fd, (const uint8_t*)&c, 1);
}
...
void uavcan_dl_set_baudrate(struct uavcan_tunnel_periph* up __attribute__((unused)), uint32_t baudrate __attribute__((unused))) {
