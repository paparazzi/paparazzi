 ...
 */
  /** \file hott_eam.h
 *  \brief Graupner HOTT electric air module description
 *
 */
...
static void hott_update_eam_msg(struct HOTT_EAM_MSG* hott_eam_msg) {
  ...
  uint16_t speed_buf = (uint16_t)(sqrt(pow(stateGetSpeedEnu_i()->x, 2) + pow(stateGetSpeedEnu_i()->y, 2)) * 36 / 10 / (1 << INT32_SPEED_FRAC));
