...
void ins_flow_update(void)
{
  ...
  */
  ...
  if (CONSTANT_ALT_FILTER) {
    ...
    OF_X[OF_V_IND] += dt * (thrust * sinf(OF_X[OF_ANGLE_IND]) / mass);
    OF_X[OF_Z_IND] += dt * OF_X[OF_Z_DOT_IND];
    OF_X[OF_Z_DOT_IND] += dt * (thrust * cosf(OF_X[OF_ANGLE_IND]) / mass - g);
    OF_X[OF_ANGLE_IND] += dt * OF_X[OF_ANGLE_DOT_IND];
    /*
  ...
  if (ins_flow.new_flow_measurement) {
    ...
    if(CONSTANT_ALT_FILTER) {
      ...
      float Z_exp_with_rate = -OF_X[OF_V_IND]*cosf(OF_X[OF_ANGLE_IND])*cosf(OF_X[OF_ANGLE_IND])/OF_X[OF_Z_IND]+OF_X[OF_ANGLE_DOT_IND];
