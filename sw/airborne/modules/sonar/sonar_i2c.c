...
#define READ_MODE_SINGLE 0x51 // Deducted from sparse random information and thereafter testing
...
#define SONAR_I2C_USE_FILTER 1 // Enable filtering for sonar per default is best, sonars tend to give noisy spiking readings
#endif
...
#define SONAR_I2C_SCALE 0.000044f  // Experimentally determined gain for famous GY-US42V2 sensor, since there is no datasheet afaik
#endif
...
void sonar_i2c_init(void)
{
  ...
  sonar_i2c.distance = (float)SONAR_I2C_MIN_RANGE; // Start with minimum range as default distance
  sonar_i2c.raw = (uint16_t)(sonar_i2c.distance / SONAR_I2C_SCALE);
...
void sonar_i2c_periodic(void)
{
  ...
  switch (sonar_i2c.status) {
    ...
    case SONAR_I2C_PARSE_DATA: {
      ...
      if (sonar_i2c.distance <= (float)SONAR_I2C_MAX_RANGE) {  //Discard non reliable readings that are out of range
        if (sonar_i2c.distance < (float)SONAR_I2C_MIN_RANGE) { sonar_i2c.distance = (float)SONAR_I2C_MIN_RANGE; }
        ...
        if (gain < 0.4f) {sonar_i2c.distance = NAN; } //The magic 0.4f is for regular ultrasonic sensors about maximum range
        #endif

        if (!isnan(sonar_i2c.distance)) { 
          sonar_i2c.distance = sonar_i2c.distance - (float)SONAR_I2C_OFFSET; // Must be applied after rotation compensation
          // Send AGL message
          ...
          if (sonar_i2c.distance > 0.0f && sonar_i2c.update_agl) {
            AbiSendMsgAGL(AGL_SONAR_I2C_ID, now_ts, sonar_i2c.distance);
          } 
        }
      } else {
        // Out of range, set to NAN
        sonar_i2c.distance = NAN;
      }
