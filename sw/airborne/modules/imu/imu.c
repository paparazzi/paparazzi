...
static void cmd_imu(shell_stream_t *sh, int argc, const char *const argv[])
{
  ...
  for (i = 0; i < IMU_MAX_SENSORS; i++) {
    if (imu.gyros[i].abi_id != 0) {
      ...
      else if(&imu.gyros[i].calibrated.scale) {                 // Deprecated, TBR
        chprintf(sh, "Using deprecated int scale !!\r\n");
        show_rates(sh, "scale_num", &imu.gyros[i].scale[0]);    // Deprecated, TBR
        show_rates(sh, "scale_den", &imu.gyros[i].scale[1]);    // Deprecated, TBR
      }
  ...
  for (i = 0; i < IMU_MAX_SENSORS; i++) {
    if (imu.accels[i].abi_id != 0) {
      ...
      else if(&imu.accels[i].calibrated.scale) {                 // Deprecated, TBR
        chprintf(sh, "Using deprecated int scale !!\r\n");
        show_vect3(sh, "scale_num", &imu.accels[i].scale[0]);    // Deprecated, TBR
        show_vect3(sh, "scale_den", &imu.accels[i].scale[1]);    // Deprecated, TBR
      }
  ...
  for (i = 0; i < IMU_MAX_SENSORS; i++) {
    if (imu.mags[i].abi_id != 0) {
      ...
      else if(&imu.mags[i].calibrated.scale) {                 // Deprecated, TBR
        chprintf(sh, "Using deprecated int scale !!\r\n");
        show_vect3(sh, "scale_num", &imu.mags[i].scale[0]);    // Deprecated, TBR
        show_vect3(sh, "scale_den", &imu.mags[i].scale[1]);    // Deprecated, TBR
      }
...
void imu_init(void)
{
  ...
  for(uint8_t i = 0; i < IMU_MAX_SENSORS; i++) {
    ...
    if(!imu.gyros[i].calibrated.scale) {  // Deprecated, TBR
 ...
 */
void imu_set_defaults_gyro(uint8_t abi_id, const struct Int32RMat *imu_to_sensor, const struct Int32Rates *neutral, const struct Int32Rates *scale, const struct FloatVect3 *scale_f)
{
 ...
 * @param scale Scale values, 0 index is multiply and 1 index is divide
 */
void imu_set_defaults_accel(uint8_t abi_id, const struct Int32RMat *imu_to_sensor, const struct Int32Vect3 *neutral, const struct Int32Vect3 *scale, const struct FloatVect3 *scale_f)
{
 ...
 * @param scale Scale values, 0 index is multiply and 1 index is divide
 */
void imu_set_defaults_mag(uint8_t abi_id, const struct Int32RMat *imu_to_sensor, const struct Int32Vect3 *neutral, const struct Int32Vect3 *scale, const struct FloatVect3 *scale_f)
{
