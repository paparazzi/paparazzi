...
void baro_bmp280_event(void)
{
  ...
  if (baro_bmp280.data_available) {
    ...
    // send ABI message
    AbiSendMsgBARO_ABS(BARO_BMP280_SENDER_ID, now_ts, baro_bmp280.pressure);
    AbiSendMsgTEMPERATURE(BARO_BMP280_SENDER_ID, baro_bmp280.temperature);
    baro_bmp280.data_available = false;
    baro_alt = pprz_isa_altitude_of_pressure((float)baro_bmp280.pressure);
    baro_alt_valid = TRUE;

#ifdef BMP280_SYNC_SEND
    int32_t up = (int32_t)(baro_alt * 100.0); //baro_bmp280.raw_pressure;
