...
struct opticflow_pmw3901_settings_t of_pmw_set;
...
static bool agl_valid(uint32_t at_ts) {
  return \
      ((at_ts - agl_ts) < of_pmw_set.agl_timeout) &&
      (agl_dist > 0.080);
}
...
static void opticflow_pmw3901_publish(int16_t delta_x, int16_t delta_y, uint32_t ts_usec) {
  ...
  int16_t flow_x_subpix = (int16_t)(of_pmw_set.subpixel_factor * flow_x);
  int16_t flow_y_subpix = (int16_t)(of_pmw_set.subpixel_factor * flow_y);
  // Derotated flow [px/s] (body-frame)
  struct FloatRates *rates = stateGetBodyRates_f();
  float flow_dy_p =  rates->p / of_pmw.rad_per_px;
  float flow_dx_q = -rates->q / of_pmw.rad_per_px;
  float flow_der_x = flow_x - flow_dx_q;
  float flow_der_y = flow_y - flow_dy_p;
  int16_t flow_der_x_subpix = (int16_t)(of_pmw_set.subpixel_factor * flow_der_x);
  int16_t flow_der_y_subpix = (int16_t)(of_pmw_set.subpixel_factor * flow_der_y);
  // Velocity
  ...
  if (agl_valid(ts_usec)) {
    vel_x = flow_der_x * of_pmw.rad_per_px * agl_dist;
    vel_y = flow_der_y * of_pmw.rad_per_px * agl_dist;
    noise = of_pmw_set.std_px * of_pmw.rad_per_px * agl_dist;
  }
...
void opticflow_pmw3901_init(void) {
  pmw3901_init(&of_pmw, &OPTICFLOW_PMW3901_SPI_DEV, OPTICFLOW_PMW3901_SPI_SLAVE_IDX);
  AbiBindMsgAGL(OPTICFLOW_PMW3901_AGL_ID, &agl_ev, agl_cb);
  of_pmw_set.sensor_angle = OPTICFLOW_PMW3901_SENSOR_ANGLE;
  of_pmw_set.subpixel_factor = OPTICFLOW_PMW3901_SUBPIXEL_FACTOR;
  of_pmw_set.std_px = OPTICFLOW_PMW3901_STD_PX;
  of_pmw_set.agl_timeout = OPTICFLOW_PMW3901_AGL_TIMEOUT_US;
}

void opticflow_pmw3901_periodic(void) {
  if (pmw3901_is_idle(&of_pmw)) {
    pmw3901_start_read(&of_pmw);
  }
}

void opticflow_pmw3901_event(void) {
  pmw3901_event(&of_pmw);
  if (pmw3901_data_available(&of_pmw)) {
    int16_t delta_x, delta_y;
    pmw3901_get_data(&of_pmw, &delta_x, &delta_y);
