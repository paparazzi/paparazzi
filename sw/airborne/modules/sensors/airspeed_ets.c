...
//use raw value for sensor set to third-party mode
  ...
  if (pprzLogFile != -1) {
    if (!log_airspeed_ets_started) {
      sdLogWriteLog(pprzLogFile, "Airspeed(m/s) GPS_fix TOW(ms) Week Lat(1e7rad) Lon(1e7rad) HMSL(mm) gpseed(cm/s) course(1e7rad) climb(cm/s)\n");
      log_airspeed_ets_started = TRUE;
    }
    else {
      sdLogWriteLog(pprzLogFile, "airspeed: %d %d %8.4f   %d %d %d   %d %d %d   %d %d %d\n",
