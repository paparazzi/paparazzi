...
static void power_dronecan_send_power_device(struct transport_tx *trans, struct link_device *dev)
{
  ...
  if(idx >= POWER_DRONECAN_CIRCUITS_MAX || !circuits[idx].set) {
    idx = 0;
  }
}
...
// #endif
  ...
  // Configure telemetry
#if PERIODIC_TELEMETRY
  register_periodic_telemetry(DefaultPeriodic, PPRZ_MSG_ID_POWER_DEVICE, power_dronecan_send_power_device);
#endif
