...
void ir_mlx_periodic( void ) {
  ...
  if (cpu_time_sec > 1) {
...
#ifdef IR_MLX_ONE_TIME_CONFIG
  else if ((cpu_time_sec > 1) && (ir_mlx_status == IR_MLX_ADDR_CHANGE)) {
    ...
    I2CTransmit(MLX_I2C_DEV, mlx_trans, MLX90614_GENERAL_ADDR, 4);
  } else
  if ((cpu_time_sec > 2) && (ir_mlx_status == IR_MLX_ADDR_ERASE)) {
  ...
  if ((cpu_time_sec > 3) && (ir_mlx_status == IR_MLX_ADDR_SET)) {
    ir_mlx_status = IR_MLX_UNINIT;
  }
#endif
}
