...
void CN_potential_heading(void){
	...
	float Distance_est;
	float angle_hor = - 0.5*stereo_fow[0] - stereo_fow[0]/size_matrix[2] + stereo_fow[0]/size_matrix[2]/2;       //calculate position angle of stereo
	float angle_hor_abs;
	...
	for (int i1=0;i1<size_matrix[0];i1++){
		  ...
		  for (int i2=0;i2<4;i2++){
			...
			if(READimageBuffer[i1*size_matrix[1]+i2*size_matrix[0]*size_matrix[2] + i3]>min_disparity){
			    ...
			    potential_obst_temp = potential_obst_temp - K_obst*(angle_hor)*exp(-c3_oa*angle_hor_abs)*exp(-c4_oa*Distance_est);//(tan(obst_width[i]+c5)-tan(c5)); 
			    potential_obst_integrated = potential_obst_integrated + K_obst*c3_oa*(fabs(angle_hor)+1)/(c3_oa*c3_oa)*exp(-c3_oa*fabs(angle_hor))*exp(-c4_oa*Distance_est);// (tan(obst_width[i]+c5)-tan(c5));
	...
	r_dot_new = -b_damp*r_old - K_goal*(heading_goal_ref)*(exp(-c1_oa*sqrt(VECT2_NORM2(pos_diff)))+c2_oa) + potential_obst;
...
void CN_potential_velocity(void){
	...
	for (int i1=0;i1<size_matrix[0];i1++){
		  ...
		  for (int i2=0;i2<size_matrix[1];i2++){
			...
			potential_obst = potential_obst + K_obst*(diff_angle)*exp(-c3_oa*fabs(diff_angle))*exp(-c4_oa*Distance_est);//(tan(obst_width[i]+c5)-tan(c5)); 
			potential_obst_integrated = potential_obst_integrated + K_obst*c3_oa*(fabs(diff_angle)+1)/(c3_oa*c3_oa)*exp(-c3_oa*fabs(diff_angle))*exp(-c4_oa*Distance_est);// (tan(obst_width[i]+c5)-tan(c5));
...
void CN_vector_escape_velocity(void){
	  ...
	  ref_roll = Total_Kan.y;
	    ...
	    #if PRINT_STUFF
		for(int i100=0;i100<36;i100++){
		  printf("%i diff_available_heading: %f available_heading: %f \n",i100,diff_available_heading[i100],available_heading[i100]);
		}
		printf("new_heading: %f  current_heading: %f  speed_pot: %f\n",new_heading,stateGetNedToBodyEulers_f()->psi,speed_pot);	
