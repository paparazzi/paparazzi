...
int32_t humid_count = 0;

struct temp_dat_t {
int32_t valid; /* data slot */
double alt;   /* altitude [m] */ 
double temp;  /* temperature [degree celsius] */
};

struct hum_dat_t {
int32_t valid; /* data slot */
double alt;   /* altitude [m] */ 
double humid; /* relative humidity [percent] */
};

struct temp_dat_t temp_dat[HEIGHT_SPAN] = { {0} };
struct hum_dat_t hum_dat[HEIGHT_SPAN] = { {0} };
...
*/
  ...
  temp_dat[temp_count].alt = alt;
  temp_dat[temp_count].temp = _temp;
  temp_count++;

  fprintf(Gplt, "plot '-' title \"TMP_STATUS\"\n");
  if (temp_count < HEIGHT_SPAN) { 
    for (i = 0; i < temp_count; i++){
      fprintf(Gplt, "%.2f %.2f\n", temp_dat[i].temp, temp_dat[i].alt);
    }
  }
  fprintf(Gplt,"e\n");

//  printf("%.2f %.2f\n", temp_dat[i].temp, temp_dat[i].alt);
  printf("%d: alt %.2f, temp %.2f\n", i, alt, _temp);
}
...
*/
  ...
  Gplt = popen("gnuplot -geometry 300x600","w");
  setlinebuf(Gplt);

  fprintf(Gplt, "set autoscale\n");
  fprintf(Gplt, "set style data linespoints\n");
  fprintf(Gplt, "set terminal x11 noraise\n");
